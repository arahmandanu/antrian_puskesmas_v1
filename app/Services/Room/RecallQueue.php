...
class ReCallQueue extends \App\Services\AbstractService
{
    ...
    public function handle()
    {
        ...
        if ($pendingExist) return Result::failure(Lang::get('messages.pending_queue', ['queue' => $pendingExist->formatAsQueueNumber()], 'id'));

        $result = $this->room->queuesCalled()
            ->whereBetween('created_at', [now()->startOfDay(), now()->endOfDay()])
            ->take(1)->first();
        if (!$result) return Result::failure(Lang::get('messages.empty_history', [], 'id'));
